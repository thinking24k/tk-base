...
* @Description: 本类是由代码生成器自动生成GroupEntity逻辑(Controller)层
* @company 
* @author yixiang.deng
...
public class GroupController extends BaseController {
	...
	* @Title: doAdd 
	* @Description: 新增操作
	* @param groupEntity  Post提交
	* @param request
	* @param response
	* @throws BusinessException  
	*/ 
	@RequestMapping(value="/doadd",method = RequestMethod.POST)
	public @ResponseBody MessageDTO doAdd(GroupEntity groupEntity,HttpServletRequest request,HttpServletResponse response) {
		// 1.服务器校验
		if(!doNullValidation(groupEntity)){ 
			return this.responseData(false, null,MessageAttribute.COMMON_ERROR_VAL_EMPTY_OBJ);
		}
		//2.业务层调用
		Integer doAdd = groupService.doAdd(groupEntity);
		//3.返回JSON数据
	...
	/**
	 * @Title: 更新操作
	 * @Description: 更新GroupEntity 表单对象
	 * @param groupEntity
	 *            表单DTO对象
	 * @param response
	 * @throws BusinessException
	...
	public @ResponseBody MessageDTO doUpdate(GroupEntity groupEntity,HttpServletRequest request,HttpServletResponse response) {	
		// 1.服务器校验
		if(!doNullValidation(groupEntity)){ 
			return this.responseData(false, null,MessageAttribute.COMMON_ERROR_VAL_EMPTY_OBJ);
		}
		...
		// 3.校验成功，进行业务处理
		Integer doUpdate = groupService.doUpdate(groupEntity);
	...
	private MessageDTO deleteById(Integer id){
		...
		// 2.校验成功，进行业务处理
		Integer doDelete = groupService.doDelete(id);
   ...
   /**
	...
	private MessageDTO removeById(Integer id){
		...
		// 2.校验成功，进行业务处理
		Integer doRemove = groupService.doRemove(id);
	...
	private MessageDTO selectById(Integer id) {
		...
		return this.responseData(true, groupEntity,MessageAttribute.COMMON_SELECT_VAL_SUC);
	}	
	...
	public @ResponseBody MessageDTO pageQuery(Criteria<GroupEntity>  groupCriteria){
		// 1.服务器校验
		if(!doNullValidation(groupCriteria)){ 
			return this.responseData(false, null,MessageAttribute.COMMON_ERROR_VAL_EMPTY_OBJ);
		}
		PageBean<GroupEntity> pageQuery = groupService.pageQuery(groupCriteria);
	...
	public @ResponseBody MessageDTO queryForList(Criteria<GroupEntity>  groupCriteria) {
		// 1.服务器校验
		if(!doNullValidation(groupCriteria)){ 
			return this.responseData(false, null,MessageAttribute.COMMON_ERROR_VAL_EMPTY_OBJ);
		}
		//查询条件
		List<GroupEntity> groupEntityList = groupService.queryForList(groupCriteria);
		return this.responseData(true, groupEntityList,MessageAttribute.COMMON_SELECT_VAL_SUC);				
	}		

    
	
	
	private boolean doUpdateValidation(GroupEntity groupEntity) {
		boolean status = true;
		// 1.空对象校验
		if (groupEntity == null) {
			status = false;
	...
	* @return
	*/
	protected <T> boolean doAddValidation(GroupEntity groupEntity ) {
		boolean status = true;
		// 1.空对象校验
		if (groupEntity == null) {
			status = false;
